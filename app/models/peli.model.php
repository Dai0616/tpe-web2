...
class PeliModel {
    ...
    function getAllPelisHome() {
        $query = $this->db->prepare("SELECT pelis.id, pelis.nombre, pelis.estreno, pelis.puntuacion, pelis.director, pelis.fk_estudio, estudio.id_nombre_fk, estudio.nombre_estudio FROM pelis INNER JOIN estudio ON pelis.fk_estudio = estudio.id_nombre_fk");
        $query->execute();
        $pelis = $query->fetchAll(PDO::FETCH_OBJ); // devuelve un arreglo de objetos
